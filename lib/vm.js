...
function getLocals(fullName, mainName, childs, context, callback, a1, a2, a3) {
	...
	if (/.*\[.*\]$/.test(fullName)) {
		...
		var name	= parts[0];
		var index	= parts[1].split(']');
		index		= index[0];

		var info	= context.getLocals(name);

		if (info) {
			callback(null, info[1][index], a1, a2, a3);
			return;
		}
	}
...
execOpcode[OPCODE.SECTION]	= function(context, callback, ast) {
	run(ast[1], context, function (err, filename) {
		...
		if (context._locals.sections) {
			if (context._locals.sections.hasOwnProperty(id)) {
				locals	= context._locals.sections[id];
			} else if (context._locals.sections.hasOwnProperty(default_id)) {
				locals	= context._locals.sections[default_id];
			}
		}

		context.section(filename, locals, callback);
	});
};
