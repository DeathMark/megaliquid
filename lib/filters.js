...
/*------------------------------- Shopify Filters ------------------------------*/
exports.img_url	= function(obj, size) {
	if (obj) {
		if (obj.src) {
			return obj.src;
		} else if (obj.image) {
...
exports.t	= function(path) {
	var getChildVal	= function(value, child) {
		var ret	= getChildValue(value, child);
	...
	var text	= getChildVal(context._locals, path.split('.'));

	if (text) {
		if (text.one && text.other) {
			var count	= 0;
			...
			text = count > 1 ? text.other : text.one;
		}

		for (var i = 1; i < args.length - 1; i++) {
			text	= text.replace('{{ ' + args[i][0] + ' }}', getChildVal(context._locals, args[i][1].split('.')));
		}
	}
...
exports.asset_url	= function(url) {
	return 'assets/' + url;
};

exports.shopify_asset_url = function(url) {
	return 'assets/' + url;
};

exports.money_without_trailing_zeros = function(money) {
	if (money) {
